...
class TerminalView: NSView {
    ...
    let composerCardView = NSView()
    let scrollView = NSScrollView()
    let textView = NSTextView()
    let inputField = NSTextField()
    var onSendMessage: ((String) -> Void)?
    var onClearRequested: (() -> Void)?
    var provider: AgentProvider = .doubao {
        didSet {
            applyTheme()
        }
    }
    ...
    private func setupViews() {
        ...
        let messageCardHeight = frame.height - composerHeight - outerPadding * 2 - sectionSpacing

        wantsLayer = true

        messageCardView.frame = NSRect(
            x: outerPadding,
            y: outerPadding + composerHeight + sectionSpacing,
            width: frame.width - outerPadding * 2,
            height: max(messageCardHeight, 120)
        )
        messageCardView.autoresizingMask = [.width, .height]
        addSubview(messageCardView)

        composerCardView.frame = NSRect(
            x: outerPadding,
            y: outerPadding,
            width: frame.width - outerPadding * 2,
            height: composerHeight
        )
        composerCardView.autoresizingMask = [.width, .maxYMargin]
        addSubview(composerCardView)

        scrollView.frame = NSRect(
            x: 8,
            y: 8,
            width: messageCardView.frame.width - 16,
            height: messageCardView.frame.height - 16
        )
        ...
        inputField.frame = NSRect(
            x: 8,
            y: 7,
            width: composerCardView.frame.width - 16,
            height: 30
        )
        ...
        composerCardView.addSubview(inputField)
        applyTheme()
    }

    private func applyTheme() {
        guard inputField.cell != nil else { return }

        let t = theme
        layer?.backgroundColor = NSColor.clear.cgColor

        messageCardView.wantsLayer = true
        messageCardView.layer?.backgroundColor = t.inputBg.withAlphaComponent(0.45).cgColor
        messageCardView.layer?.cornerRadius = 18
        messageCardView.layer?.borderWidth = 1
        messageCardView.layer?.borderColor = t.separatorColor.cgColor

        composerCardView.wantsLayer = true
        composerCardView.layer?.backgroundColor = t.inputBg.cgColor
        composerCardView.layer?.cornerRadius = 16
        composerCardView.layer?.borderWidth = 1
        composerCardView.layer?.borderColor = t.separatorColor.cgColor

        textView.textColor = t.textPrimary
        textView.font = t.font
        textView.insertionPointColor = t.textPrimary
        textView.linkTextAttributes = [
            .foregroundColor: t.accentColor,
            .underlineStyle: NSUnderlineStyle.single.rawValue
        ]
